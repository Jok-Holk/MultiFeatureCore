...
| Version | Summary |
|---|---|
| 5.0.3 | `/coreupdate` now requires an explicit delay (`now\|0\|1-10`) and broadcasts a full localized countdown instead of updating immediately. Add a minimum-cooldown floor to Ragnarok/Ignis/Excalibur/Spear so a quick tap can no longer spam a skill for near-zero cooldown (fixes Spear's lunge being spammable). Ragnarok rewritten from an animated sweep to a single fixed 3D box directly in front of the player, 10 blocks tall starting at the player's feet -- fixed a `Vector.crossProduct()` aliasing bug that was the actual root cause of its hit area looking diagonal/off to one side; felled logs now give the correct wood type/amount straight into the inventory; reach buffed, damage rebalanced down. Ignis's destruction radius/length nerfed several times over (damage unchanged); ore blocks now give already-smelted ingots (Fortune-aware quantity) straight into the inventory instead of raw ore. Nothan now strips vanilla's real crossbow charge state every tick instead of every 5, and item model swaps force a client resync -- fixes a real vanilla bolt occasionally firing instead of the custom effect, and the held item getting stuck centered on screen after firing. Spear and Excalibur damage rebalanced down |
