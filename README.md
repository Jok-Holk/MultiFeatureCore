...
### Rank System
Five ranks: **GUEST â†’ BUILDER â†’ ADMIN â†’ OWNER â†’ DEVELOPER**
...
Personal toggle â€” shows player name, rank, and coordinates. Updates every 5 seconds.
...
### God Mace (`/godmace`)
Right-click to launch upward. Falling on a player below executes divine judgment. Item is owner-locked â€” anyone else who picks it up gets kicked.

### Abyssal Sovereign (`/abyssal`)
A sea-themed trident. Throws at 3Ã— normal speed with extreme base damage. On hit: lightning strike + explosion + splash AoE damage to surrounding entities. Returns to owner after hitting a block. Hit a player in Survival: instant execution. Owner-locked like the God Mace.

**Wet-condition bonus** â€” if the target is in water or exposed to rain:
- Direct hit damage doubles (40 â†’ 80)
- Splash AoE expands from 3Ã—3 to 6Ã—6 blocks
- Explosion radius doubles; 6 fireworks instead of 3

### Glass Shortcut (`/glass`)
Places a glass block at your feet instantly â€” for temporary scaffolding.

### Day Length (`/daylength`)
Control how long a Minecraft day takes in real minutes.
- `/daylength 45` â€” set to 45 minutes per cycle
- `/daylength` â€” show current setting
- `/daylength reset` â€” restore vanilla default (20 minutes)
...
| `/scoreboard [on\|off]` | Toggle personal scoreboard | â€” |
...
| Rank | Permissions |
|---|---|
| GUEST | Travel, checkpoints â€” forced Adventure mode |
...
> **Note:** Uses `maven-jar-plugin` (not shade). The plugin has no runtime dependencies â€” Paper API is `provided`.
...
| Version | Summary |
|---|---|
| 4.4.0 | Abyssal Sovereign sound rebalance: IMPACT replaces THUNDER, Elder Guardian dominant; remove rank test code |
...
| 4.2.5 | Add FAVS (`voxelsniper.*`) permissions; fix JAR build (shade â†’ jar-plugin) |
| 4.2.0 | Add Abyssal Sovereign trident; fix deprecated `DO_DAYLIGHT_CYCLE` API |
| 4.1.0 | Add `/glass` and `/daylength` |
| 4.0.0 | Major rewrite â€” HeightLock, Measure, GodMace, rank system overhaul |
| 2.1.1 | Fix scoreboard global state |
| 2.1.0 | Rename `/menu` â†’ `/travel`, rework travel system |
